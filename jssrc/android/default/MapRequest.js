...
function successcallback(location) {
    ...
    httpRequestPlace.onReadyStateChange = function() {
        HandleResponseLoc(httpRequestPlace)
    };
}
...
function successcallbackNearby() {
    ...
    httpRequestPoi.send();
    httpRequestPoi.onReadyStateChange = function() {
        HandleResponsePoi(httpRequestPoi)
    };
}
...
function getEvent() {
    ...
    httpRequestEvent.onReadyStateChange = function() {
        HandleResponseEvent(httpRequestEvent)
    };
}
    ...
    if (objPoi.readyState == 4) {
        if (objPoi.response != null && objPoi.response != "") {
            ...
            if ('error' in jsonObj) {
                ...
                var lengthObject = Object.keys(poi).length;
                segEvent.widgetDataMap = {
                    imgEvents: "imgEvents",
                    lblNama: "lblNama",
                    lblDatenTime: "lblDatenTime",
                    lblDistance: "lblDistance",
                    lblPrice: "lblPrice"
                };
                var datc = [];
